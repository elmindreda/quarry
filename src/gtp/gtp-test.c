...
{
  fprintf((FILE *) user_data, "%s", command);
...
main(int argc, char *argv[], char *envp[])
{
  ...
  if (argc > 1) {
    ...
    assert(pipe(engine_in_pipe) == 0);
    assert(pipe(engine_out_pipe) == 0);
	...
	if (descriptor != STDERR_FILENO)
      ...
      assert(dup2(engine_in_pipe[0], STDIN_FILENO) == STDIN_FILENO);
      assert(dup2(engine_out_pipe[1], STDOUT_FILENO) == STDOUT_FILENO);
      ...
      if (child_pid > 0) {
	FILE *engine_in_stream = fdopen(engine_in_pipe[1], "wb");
	...
	while (client_is_alive) {
	  ...
	  if (FD_ISSET(engine_out, &in_set)) {
		...
		int random_chunk_length
		  = 1 + random() % (((bytes_read - processed_length) * 3) / 2);
